...
public class GraphAlgorithms {
  ...
  public static <V, E> void breadthFirstTraversal(AdjacencyMapGraph<V, E> graph, Vertex<V> start) {
    Queue<Vertex<V>> queue = new LinkedList<>();

    Map<Vertex<V>, Boolean> visited =  new ProbeHashMap<>();
    ...
    while (!queue.isEmpty()) {
      ...
      System.out.println("Visited: " + currentVertex.getElement());
      ...
      for (Edge<E> edge : graph.outgoingEdges(currentVertex)) {
        ...
        if (!visited.get(oppositeVertex)) {
          visited.put(oppositeVertex, true);
  ...
  public static <V, E> boolean hasCycle(AdjacencyMapGraph<V, E> graph) {
    Set<Vertex<V>> visited = new HashSet<>();
    for (Vertex<V> vertex : graph.vertices()) {
      if (!visited.contains(vertex)) {
        if (hasCycle(graph, vertex, null, visited)) {
          return true;
        }
      }
    }
  ...
  private static <V, E> boolean hasCycle(AdjacencyMapGraph<V, E> graph, Vertex<V> vertex, Vertex<V> parent, Set<Vertex<V>> visited) {
    visited.add(vertex);

    for (Edge<E> edge : graph.outgoingEdges(vertex)) {
      Vertex<V> oppositeVertex = graph.opposite(vertex, edge);
      if (!visited.contains(oppositeVertex)) {
        if (hasCycle(graph, oppositeVertex, vertex, visited)) {
          return true;
        }
      } else if (!oppositeVertex.equals(parent)) {
        return true;
      }
    }
