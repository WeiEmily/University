...
//    }
    ...
    public int countExt() {
        ...
        if(root == null || numChildren(root) == 0){
            ...
            count += countPre(left(root)) + countPre(right(root));
        }
        return count;
    }
    public int countPre(Position<E> p) {
        int num= 0;
        Node<E> checkNode = validate(p);
        if(checkNode != null && numChildren(checkNode) == 0){
            return 1 + num;
        }else{
            if(checkNode.left != null){
                return countPre(left(p));
            };
            if(checkNode.right != null){
                return countPre(right(p));
            };
        }
        return num;
    }
