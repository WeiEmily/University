...
public class GraphAlgorithms {
   ...
   */
  public static <V, E> void DFS(Graph<V, E> g, CS211Lab11.Vertex<V> u,
                                Set<CS211Lab11.Vertex<V>> known, CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<E>> forest) {
    known.add(u); // u has been discovered
    for (Edge<E> e : g.outgoingEdges(u)) { // for every outgoing edge from u
      CS211Lab11.Vertex<V> v = g.opposite(u, e);
      if (!known.contains(v)) {
   ...
   *               started at u.
   */
  public static <V, E> CS211Lab11.PositionalList<Edge<E>> constructPath(Graph<V, E> g, CS211Lab11.Vertex<V> u, CS211Lab11.Vertex<V> v,
                                                                        CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<E>> forest) {
    CS211Lab11.PositionalList<Edge<E>> path = new LinkedPositionalList<>();
    if (forest.get(v) != null) { // v was discovered during the search
      CS211Lab11.Vertex<V> walk = v; // we construct the path from back to front
      while (walk != u) {
   ...
   *         included in the map).
   */
  public static <V, E> CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<E>> DFSComplete(Graph<V, E> g) {
    Set<CS211Lab11.Vertex<V>> known = new HashSet<>();
    CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<E>> forest = new CS211Lab11.ProbeHashMap<>();
    for (CS211Lab11.Vertex<V> u : g.vertices())
      if (!known.contains(u))
   ...
   *               and adds discovery graph edges to the forest.
   */
  public static <V, E> void BFS(Graph<V, E> g, CS211Lab11.Vertex<V> s,
                                Set<CS211Lab11.Vertex<V>> known, CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<E>> forest) {
    CS211Lab11.PositionalList<CS211Lab11.Vertex<V>> level = new LinkedPositionalList<>();
    known.add(s);
    level.addLast(s); // first level includes only s
    while (!level.isEmpty()) {
      CS211Lab11.PositionalList<CS211Lab11.Vertex<V>> nextLevel = new LinkedPositionalList<>();
      for (CS211Lab11.Vertex<V> u : level)
   ...
   *         included in the map).
   */
  public static <V, E> CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<E>> BFSComplete(Graph<V, E> g) {
    CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<E>> forest = new CS211Lab11.ProbeHashMap<>();
    Set<CS211Lab11.Vertex<V>> known = new HashSet<>();
    for (CS211Lab11.Vertex<V> u : g.vertices())
      if (!known.contains(u))
  ...
  public static <V, E> void transitiveClosure(Graph<V, E> g) {
    for (CS211Lab11.Vertex<V> k : g.vertices())
      for (CS211Lab11.Vertex<V> i : g.vertices())
        // verify that edge (i,k) exists in the partial closure
        if (i != k && g.getEdge(i, k) != null)
          for (CS211Lab11.Vertex<V> j : g.vertices())
   ...
   */
  public static <V, E> CS211Lab11.PositionalList<CS211Lab11.Vertex<V>> topologicalSort(Graph<V, E> g) {
    // list of vertices placed in topological order
    CS211Lab11.PositionalList<CS211Lab11.Vertex<V>> topo = new LinkedPositionalList<>();
    // container of vertices that have no remaining constraints
    Stack<CS211Lab11.Vertex<V>> ready = new LinkedStack<>();
    // map keeping track of remaining in-degree for each vertex
    CS211Lab11.Map<CS211Lab11.Vertex<V>, Integer> inCount = new CS211Lab11.ProbeHashMap<>();
    for (CS211Lab11.Vertex<V> u : g.vertices()) {
    ...
    while (!ready.isEmpty()) {
      CS211Lab11.Vertex<V> u = ready.pop();
      topo.addLast(u);
   ...
   */
  public static <V> CS211Lab11.Map<CS211Lab11.Vertex<V>, Integer> shortestPathLengths(Graph<V, Integer> g, CS211Lab11.Vertex<V> src) {
    // d.get(v) is upper bound on distance from src to v
    CS211Lab11.Map<CS211Lab11.Vertex<V>, Integer> d = new CS211Lab11.ProbeHashMap<>();
    // map reachable v to its d value
    CS211Lab11.Map<CS211Lab11.Vertex<V>, Integer> cloud = new CS211Lab11.ProbeHashMap<>();
    // pq will have vertices as elements, with d.get(v) as key
    AdaptablePriorityQueue<Integer, CS211Lab11.Vertex<V>> pq;
    pq = new HeapAdaptablePriorityQueue<>();
    // maps from vertex to its pq locator
    CS211Lab11.Map<CS211Lab11.Vertex<V>, Entry<Integer, CS211Lab11.Vertex<V>>> pqTokens;
    pqTokens = new CS211Lab11.ProbeHashMap<>();

    // for each vertex v of the graph, add an entry to the priority queue, with
    // the source having distance 0 and all others having infinite distance
    for (CS211Lab11.Vertex<V> v : g.vertices()) {
      if (v == src)
        d.put(v, 0);
      else
    ...
    while (!pq.isEmpty()) {
      Entry<Integer, CS211Lab11.Vertex<V>> entry = pq.removeMin();
      int key = entry.getKey();
      CS211Lab11.Vertex<V> u = entry.getValue();
   ...
   */
  public static <V> CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<Integer>> spTree(Graph<V, Integer> g, CS211Lab11.Vertex<V> s, CS211Lab11.Map<CS211Lab11.Vertex<V>, Integer> d) {
    CS211Lab11.Map<CS211Lab11.Vertex<V>, Edge<Integer>> tree = new CS211Lab11.ProbeHashMap<>();
    for (CS211Lab11.Vertex<V> v : d.keySet())
      if (v != s)
        for (Edge<Integer> e : g.incomingEdges(v)) { // consider INCOMING edges
          CS211Lab11.Vertex<V> u = g.opposite(v, e);
          int wgt = e.getElement();
   ...
   * order).
   */
  public static <V> CS211Lab11.PositionalList<Edge<Integer>> MST(Graph<V, Integer> g) {
    ...
    CS211Lab11.Partition<CS211Lab11.Vertex<V>> forest = new Partition<>();
    // map each vertex to the forest position
    Map<CS211Lab11.Vertex<V>, CS211Lab11.Position<CS211Lab11.Vertex<V>>> positions = new ProbeHashMap<>();
    ...
    while (tree.size() != size - 1 && !pq.isEmpty()) {
      ...
      CS211Lab11.Vertex<V>[] endpoints = g.endVertices(edge);
      CS211Lab11.Position<CS211Lab11.Vertex<V>> a = forest.find(positions.get(endpoints[0]));
